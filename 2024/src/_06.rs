...
fn find_initial_guard(data: &[String]) -> ((usize, usize), Direction) {
    data.iter().enumerate().find_map(|(i, line)| {
    ...
    for i in 0..data.len() {
        for j in 0..data[0].len() {
            ...
            loop {
                ...
                if (new_position.0, new_position.1) == (i, j) || data[new_position.0].chars().nth(new_position.1) == Some('#') {
...
pub async fn run() -> Result<(), Box<dyn std::error::Error>> {
    let input_data = get_input_data(6).await?;
